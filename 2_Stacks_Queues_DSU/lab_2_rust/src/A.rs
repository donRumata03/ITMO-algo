...
#[derive(Default)]
struct MinStack<T: PartialOrd + Ord + Copy> {
    data: Vec<T>,
    prefix_min: Vec<T>
}

impl<T: PartialOrd + Ord + Copy> MinStack<T> {
    // fn new() {
    ...
    fn push(&mut self, element: T) {
        ...
        self.prefix_min.push(
            *(self.prefix_min.last().iter()
                .chain(
                    self.data.last().iter()
                ).min().unwrap()).clone()
        );
...
fn main() {
    ...
    for _ in 0..n {
        match input.next::<usize>() {
            1 => stack.push(input.next::<i32>()),
            2 => {stack.pop();},
            3 => output.println(stack.get_min().unwrap()),
            _ => panic!()
        }
    }
}
