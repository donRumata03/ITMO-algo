...
struct DSUNode<T: Combinable + ForSingleIndex> {
	reference: usize,
	size: usize,
	property: T
}

struct DSU<T: Combinable + ForSingleIndex> {
	data: Vec<DSUNode<T>>
}

impl<T: Combinable + ForSingleIndex> DSU<T> {
	fn new(n: usize) -> DSU<T> {
		DSU {
			data: (0..n).map(|i| DSUNode { property: T::for_single_index(i), reference: i, size: 1 }).collect()
		}
	}
	...
	fn find(&mut self, mut index: usize) -> usize {
		...
		self.data[index].reference = index;

		index
	}

	fn unite(&mut self, left_index: usize, right_index: usize) {
		let left_repr = self.find(left_index);
		let right_repr = self.find(right_index);
		...
		unsafe {
			...
			big_node.property = T::combine(&big_node.property, &small_node.property);

		}
	}
}

impl<T: Combinable + ForSingleIndex> Index<usize> for DSU<T> {
	type Output = T;
...
struct NMinMaxDSUData {
	n: usize,
	min: usize,
	max: usize
}

impl ForSingleIndex for NMinMaxDSUData {
	fn for_single_index(element: usize) -> Self {
		NMinMaxDSUData {
			n: 1,
			min: element + 1,
...
impl Combinable for NMinMaxDSUData {
	fn combine(left: &Self, right: &Self) -> Self {
		NMinMaxDSUData {
			n: left.n + right.n,
...
fn main() {
	...
	let dsu_size: usize = input.next().unwrap().unwrap().parse::<usize>().unwrap();

	let mut dsu = DSU::<NMinMaxDSUData>::new(dsu_size);
	...
	'main_loop: for this_line in input {
		...
		let mut args = splitted.map(|s| s.parse::<usize>().unwrap());

		match query_name {
			"get" => {
				let index = args.next().unwrap() - 1;
				let repr = dsu.find(index);
				let NMinMaxDSUData{ n, min, max } = dsu[repr];
				output.println(format!("{} {} {}", min, max, n));
			},
			"union" => {
