...
impl Tokenizer {
	fn new() -> Tokenizer {
		...
		Tokenizer { seqs: input.next_line().split_whitespace().map(|s| String::from(s)).collect(), index: 0 }
	}
...
impl Iterator for Tokenizer {
	...
	fn next(&mut self) -> Option<Self::Item> {
		match self.seqs.get(self.index) {
			None => None,
			Some(s) => {
				self.index += 1;
				Some(match &s[..] {
					...
					"-" => Token::Minus,
					"*" => Token::Multiply,
					num => { Token::Number(num.parse::<i64>().unwrap()) }
				})
			}
		}
	}
...
fn main() {
	...
	tk.for_each(|token| {
		match token {
			Token::Plus => {
				if let Token::Number(last) = stack.pop().unwrap() {
					if let Token::Number(pre_last) = stack.pop().unwrap() {
						stack.push(Token::Number(pre_last + last));
					}
				}
			}
			Token::Minus => {
	...
	if let Token::Number(n) = stack.pop().unwrap() {
		println!("{}", n);
	}
}
