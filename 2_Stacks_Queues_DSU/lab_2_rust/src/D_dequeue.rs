...
/// The invariant is than
///
/// head.len() - 1 <= tail.len() <= head.len(),
///
/// so elements are inserted in tail's front,
/// sometimes moves of tail's front to heads back are happening
/// (or the opposite thing)
#[derive(Default)]
struct MidInsertableQueue {
	head: VecDeque<usize>,
	tail: VecDeque<usize>,
}

impl MidInsertableQueue {

	fn manage_middle(&mut self) {
	...
	fn push_back(&mut self, new_value: usize) {
		self.tail.push_back(new_value);
		self.manage_middle();
	}

	fn push_mid(&mut self, new_value: usize) {
		self.tail.push_front(new_value);
		self.manage_middle();
	}

	fn pop_front(&mut self) -> usize {
		let result = if !self.head.is_empty() { self.head.pop_front().unwrap() } else { self.tail.pop_front().unwrap() };
		self.manage_middle();
...
fn main() {
	...
	let mut list = MidInsertableQueue::default();
