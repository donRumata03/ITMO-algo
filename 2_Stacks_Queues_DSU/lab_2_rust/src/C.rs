...
fn main() {
	...
	let mut id_data = vec![None; 100_005]; // PeopleBefore at coming + passed people at coming
	let mut people_passed = 0;

	for _ in 0..n {
		match input.next::<i32>() {
			1 => {
				let id: i32 = input.next::<i32>() - 1;
				id_data[id as usize] = Some(people_passed + queue.len());
				queue.push_back(id);
			},
			2 => {
				people_passed += 1;
			...
			4 => {
				let q: i32 = input.next::<i32>() - 1;
				output.println(id_data[q as usize].unwrap() - people_passed);
			},
			5 => {
				output.println(queue.front().unwrap() + 1);
			},
			_ => println!("WTF?!")
		}
	}
