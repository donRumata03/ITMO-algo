...
fn levenshtein_distance(s1: Vec<char>, s2: Vec<char>) -> usize {
	// dp[i][j]: ans for (s1[0..i], s2[0..j])
	let mut dp = vec![vec![0_usize; s2.len() + 1]; s1.len() + 1];

	for i in 0..s1.len() + 1 { dp[i][0] = i; }
	for j in 0..s2.len() + 1 { dp[0][j] = j; }
	...
	for i in 1..=s1.len() {
		for j in 1..=s2.len() {
			...
			let it1 = once(dp[i - 1][j - 1] + if s1[i - 1] == s2[j - 1] { 0 } else { 1 });
			let it_initial = initial.into_iter();

			dp[i][j] = it1.chain(it_initial)
				.min().unwrap();
		}
	}
...
fn main() {
	...
	let s1 = input.next_line();
	let s2 = input.next_line();

	println!("{}", levenshtein_distance(s1.chars().collect(), s2.chars().collect()));
}